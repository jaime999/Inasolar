...
def getRangeData(startDate, endDate, location):
    rangeData = pd.read_sql(f"""SELECT d.Hour, CONVERT(varchar(10), d.Date, 23) AS Fecha, h.*, d.*,g.Power, ho.* 
...
def getRangeSimulationForecast(simulationParameters, predictionStartDate, predictionEndDate, consumerInputsValue, generatorInputsValue, similarDaysTab,
    ...
    rangeDataConsumer = getRangeData(
        similarDaysStartDate, similarDaysEndDate, locationConsumer)
    rangeDataGenerator = getRangeData(
        similarDaysStartDate, similarDaysEndDate, locationGenerator)
