...
class simulator():
    ...
    PERCENTAGE_RENEWABLES_PARAMS = pd.read_sql(
            "SELECT IdParameter, Type FROM AllocationParameters WHERE ParameterType = 'sunburstChildData'", GenericCode.engine).groupby('Type')
    ...
    def __init__(self):
        ...
        self._generator_max_power = 150  # kW
        ...
        self.biogas_generation_percentage = 100

        # Biogas costs
        self._bio_kw_installation_cost = 6200  # €/kW
        self.bio_installation_cost = self.generator_max_power * self.bio_kw_installation_cost  # €
        self.bio_generation_mean_cost = 25  # cents/kWh
        self.bio_amortization_period = 12  # Years

        # Photovoltaic variables
        self._photovoltaic_power = 150  # kW
        self.max_demand = 500
        self.pvFarmsInstalledPower = 200  # kW
        # PV Costs
        self._pv_kw_installation_cost = 1210  # €/kW
        ...
        # Eolic variables
        self._wind_turbine_power = 100  # kW
        ...
        # Acuatic variables?
        self.initial_upper_tank_volume = 12000
        self._upper_tank_volume = 12000  # m3(cubic meters)
        self._lower_tank_volume = 12000  # m3(cubic meters)
        ...
        self._turbine_power = 150  # kW/h
        self._pump_power = 150  # kW/h
        ...
        # Acuatic costs
        self._hydraulic_kw_installation_cost = 1620  # €/kW
        self._hydraulic_deposit_installation_cost = 24.35  # €/m3
    ...
    def range_simulation(self, start_day="2022-12-01", end_day="2023-01-01", location={}, parameters={}, with_failures=True):
        ...
        if "weather_data" not in self.cache:
            self.cache["weather_data"] = pd.read_sql(f"""SELECT d.Hour, CONVERT(varchar(10), d.Date, 23) AS Fecha,
                               h.windspeed_10m,h.temperature_2m, d.Date,g.Power, e.Price, e.Surplus
                               ...
                               and g.location = {location['Location']}
                               and (d.Date >= CAST('{start_day}' as date) and d.date  <= CAST('{end_day}' as date))
                               ORDER BY d.Date """, GenericCode.engine)
        # FETCHING PHOTOVOLTAIC GENERATION DATA FROM ALL FARMS
        if "generation_data" not in self.cache:
            self.cache["generation_data"]= pd.read_sql(f"""SELECT l.Name, d.Hour, CONVERT(varchar(10), d.Date, 23) as Fecha,
        ...
        while current_date < final_date:
            ...
            #esta es la query para seleccionar un dia en concreto
            df_query = f'Date >= "{current_date.strftime("%Y-%m-%d")}" and Date < "{(current_date + dt.timedelta(days=1)).strftime("%Y-%m-%d")} "'
            day_weather_data = self.cache["weather_data"].query(df_query)
            day_generation_data = self.cache["generation_data"].query(df_query)
            try:
                general_table = pd.concat([general_table, self.getDailyAssignmentHistorical(objective_date=current_date.strftime("%Y-%m-%d"),
                                                                                                location=location, previous_day=general_table, failures=next_hours,
                                                                                                parameters=parameters,day_weather_data=day_weather_data,
                                                                                                day_generation_data=day_generation_data)], ignore_index=True)
    ...
    def getDailyAssignmentHistorical(self, objective_date="2021-12-31", location={}, previous_day=None, failures=None, parameters={},day_weather_data=None,day_generation_data=None):
        objective_date = parser.parse(objective_date)
        return self.getDailyAssignment(day_weather_data, day_generation_data, objective_date, previous_day, failures, parameters)
    ...
    def getDailyAssignment(self, day_data, generation_of_day, objective_date, previous_day=None, failures=None, parameters={}):
        ...
        self.simulation_time += time.time() - t1
    ...
    def get_hour_assignment(self, hour, date, previous_hour, day_data, generation_by_hour, failures, parameters={}, repeated=False):
        ...
        table["PotDemUni"] = float(
            day_data["Power"].values[hour] / GenericCode.MAX_DEMAND)

        table["PotDem"] = float(table["PotDemUni"] * self.max_demand)
    ...
    def setCommonRenewableData(self, modified, table, previous_hour):
        ...
        table['PotDem-Grid' + modified]        =  (table['Grid' + modified] / table['RenewablesPowerWithGrid' + modified]) * table['PotDem']

        percentage = table['PotBio3' + modified] / table['RenewablesPowerWithGrid' + modified]
        table['PotBombeo2-PotBio3' + modified] =  percentage * table['PotBombeo2' + modified]
        table['PotDem-PotBio3' + modified]     =  percentage * table['PotDem']
        table['Surplus-PotBio3' + modified]    =  percentage * table['Surplus'+ modified]


        percentage = table['PotEol' + modified] / table['RenewablesPowerWithGrid' + modified]
        table['PotBombeo2-PotEol' + modified]  =  percentage * table['PotBombeo2' + modified]
        table['PotDem-PotEol' + modified]      =  percentage * table['PotDem']
        table['Surplus-PotEol' + modified]     =  percentage * table['Surplus' + modified]

        percentage = table['PotFV' + modified] / table['RenewablesPowerWithGrid' + modified]
        table['PotBombeo2-PotFV' + modified]   =  percentage * table['PotBombeo2' + modified]
        table['PotDem-PotFV' + modified]       =  percentage * table['PotDem']
        table['Surplus-PotFV' + modified]      =  percentage * table['Surplus' + modified]
        
        table['PotDem-PotTurbina2' + modified] =  (table['PotTurbina2' + modified] / table['RenewablesPowerWithGrid' + modified]) * table['PotDem']
        ...
        self.biogas += time.time()-t1
        return table
    
    def getRenewablesPercentagePower(self, modified, table):
        # Se agrupan por cada tipo de renovable
        for parentName, typeParams in simulator.PERCENTAGE_RENEWABLES_PARAMS:
    ...
    def getScenariosAPI(self,originalValues):
        ...
        return scenariosIntervals,totalLengthScenarios
    
    def optimizeParametersAPI(self,optimizeParameters,originalValues,start_day="2022-12-01", end_day="2022-12-02", location={}, simulationParameters={}, with_failures=True,original = None):
        scenariosIntervals,totalLengthScenarios= self.getScenariosAPI(originalValues)
        simulations = []
        index = 0
        for scenario in scenariosIntervals:
            my_simulator_copy = copy.deepcopy(original)

            scenarioParameters = scenario[0].keys()
            for combination in scenario:
                my_simulator_copy.setSimulatorParametersAPI(
                    scenarioParameters, combination, optimizeParameters)
                ...
                summary.append(
                    my_simulator_copy.addResourceParameters(optimizeParameters['IdParameter']))
    ...
    def optimizeParameters(self, optimizeParameters, originalValues, start_day="2022-12-01", end_day="2022-12-02", location={}, simulationParameters={}, with_failures=True):
        scenarios = pd.read_sql(
            'SELECT * FROM OptimizationIntervals ORDER BY OptimizationOrder', GenericCode.engine)
        simulations = []
        scenariosIntervals = []
    ...
    def setSimulatorParametersAPI(self, parameters, combination, optimizeParameters):
        for parameter in parameters:
            # Se obtienen las ids de los parámetros que cambian el mismo porcentaje (p.e. en biogas volumen digestor y gas inicial)
            simulatorParametersIds = optimizeParameters[optimizeParameters['IntervalParameter']
                                                        == parameter]['IdParameter']
            for simulatorParameter in simulatorParametersIds:
                setattr(self, simulatorParameter,
                        self.__getattribute__(simulatorParameter)*(1+combination[parameter]))
    ...
    def upper_tank_volume(self, nuevo_valor):
        ...
        self.hydraulic_installation_cost = ((self.turbine_power + self.pump_power) * self.hydraulic_kw_installation_cost + (
            self.lower_tank_volume + self._upper_tank_volume)*self.hydraulic_deposit_installation_cost)*1.5
    ...
    #Costes Fotovoltaica
    @property
    def photovoltaic_power(self):
        return self._photovoltaic_power

    @photovoltaic_power.setter
    def photovoltaic_power(self, nuevo_valor):
        self._photovoltaic_power = nuevo_valor
        self.pv_installation_cost = nuevo_valor * self.pv_kw_installation_cost

    @property
    def pv_kw_installation_cost(self):
        return self._pv_kw_installation_cost

    @pv_kw_installation_cost.setter
    def pv_kw_installation_cost(self, nuevo_valor):
        self._pv_kw_installation_cost = nuevo_valor
        self.pv_installation_cost = self.photovoltaic_power * nuevo_valor
    
    #Costes eolica
    @property
    def wind_turbine_power(self):
        return self._wind_turbine_power

    @wind_turbine_power.setter
    def wind_turbine_power(self, nuevo_valor):
        self._wind_turbine_power = nuevo_valor
        self.eol_installation_cost = nuevo_valor * self.eol_kw_installation_cost

    @property
    def eol_kw_installation_cost(self):
        return self._eol_kw_installation_cost

    @eol_kw_installation_cost.setter
    def eol_kw_installation_cost(self, nuevo_valor):
        self._eol_kw_installation_cost = nuevo_valor
        self.eol_installation_cost =  self.wind_turbine_power * nuevo_valor
    ...
    @turbine_power.setter
    def turbine_power(self, nuevo_valor):
        self._turbine_power = nuevo_valor
        self.hydraulic_installation_cost = ((nuevo_valor + self.pump_power) * self.hydraulic_kw_installation_cost + (
            self.lower_tank_volume + self.upper_tank_volume)*self.hydraulic_deposit_installation_cost)*1.5
    ...
    @pump_power.setter
    def pump_power(self, nuevo_valor):
        self._pump_power = nuevo_valor
        self.hydraulic_installation_cost = ((self.turbine_power + nuevo_valor) * self.hydraulic_kw_installation_cost + (
            self.lower_tank_volume + self.upper_tank_volume)*self.hydraulic_deposit_installation_cost)*1.5

    @property
    def hydraulic_kw_installation_cost(self):
        return self._hydraulic_kw_installation_cost

    @hydraulic_kw_installation_cost.setter
    def hydraulic_kw_installation_cost(self, nuevo_valor):
        self._hydraulic_kw_installation_cost = nuevo_valor
        self.hydraulic_installation_cost = ((self.turbine_power + nuevo_valor) * nuevo_valor + (
            self.lower_tank_volume + self.upper_tank_volume)*self.hydraulic_deposit_installation_cost)*1.5

    @property
    def lower_tank_volume(self):
        return self._lower_tank_volume

    @lower_tank_volume.setter
    def lower_tank_volume(self, nuevo_valor):
        self._lower_tank_volume = nuevo_valor
        self.hydraulic_installation_cost = ((self.turbine_power + self.pump_power) * self.hydraulic_kw_installation_cost + (
            nuevo_valor + self.upper_tank_volume)*self.hydraulic_deposit_installation_cost)*1.5
    
    @property
    def hydraulic_deposit_installation_cost(self):
        return self._hydraulic_deposit_installation_cost

    @hydraulic_deposit_installation_cost.setter
    def hydraulic_deposit_installation_cost(self, nuevo_valor):
        self._hydraulic_deposit_installation_cost = nuevo_valor
        self.hydraulic_installation_cost = ((self.turbine_power + self.pump_power) * self.hydraulic_kw_installation_cost + (
            nuevo_valor + self.upper_tank_volume)*nuevo_valor)*1.5

    # Biogas variables
    @property
    def generator_max_power(self):
        return self._generator_max_power

    @generator_max_power.setter
    def generator_max_power(self, nuevo_valor):
        self._generator_max_power = nuevo_valor
        self.bio_installation_cost = nuevo_valor * self.bio_kw_installation_cost

    @property
    def bio_kw_installation_cost(self):
        return self._bio_kw_installation_cost

    @bio_kw_installation_cost.setter
    def bio_kw_installation_cost(self, nuevo_valor):
        self._bio_kw_installation_cost = nuevo_valor
        self.bio_installation_cost = self.generator_max_power * nuevo_valor
